...
const Arpy: React.FC = () => {
    ...
    // Ref for ArpyCanvas
    const arpyCanvasRef = useRef<ArpyCanvasHandle>(null);
    // Handler for note on
    const handleNoteOn = (note: number, velocity: number) => {
        window.inlet_5_emanator = 'Arpy';
        setMidiData({ note, velocity, isCC: false });
        if (arpyCanvasRef.current) {
            arpyCanvasRef.current.callKasmFunction('update_canvas_data', { pitch: note, velocity, cc: false });
        }
    };
    ...
    const handleNoteOff = (note: number) => {
        ...
        if (arpyCanvasRef.current) {
            arpyCanvasRef.current.callKasmFunction('update_canvas_data', { pitch: note, velocity: 0, cc: false });
        }
    };
    return (
        <div className="kasm-landing-container">
            ...
            <p>
                ...
                <ArpyCanvas ref={arpyCanvasRef} title="Arpy Canvas 1" midiData={midiData} />
